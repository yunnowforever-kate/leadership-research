"""통합 분석 에이전트: Claude API로 4개 섹션 한국어 콘텐츠 생성"""
import json
import os
import anthropic
...
SECTION_GUIDES = {
    "의미": {
        "title": "리더십 모델링 & 리더 성장 로드맵의 의미",
        "instruction": (
            "리더십 모델링과 리더 성장 로드맵의 정의, 중요성, 시대적 배경을 설명하라. "
            "A4 1~1.5페이지 분량으로 작성하며, 하위 항목: 1.1 정의, 1.2 왜 중요한가, 1.3 시대적 배경."
        ),
    },
    "방법": {
        "title": "만드는 방법",
        "instruction": (
            "리더십 모델링 및 성장 로드맵을 설계하는 원칙, 단계별 프로세스, 핵심 체크포인트를 설명하라. "
            "A4 2~3페이지 분량. 하위 항목: 2.1 설계 원칙, 2.2 단계별 프로세스, 2.3 핵심 체크포인트."
        ),
    },
    "사례": {
        "title": "AI 시대에 맞는 대표 사례",
        "instruction": (
            "AI 시대에 적합한 리더십 모델링 사례 3~5개와 리더 성장 로드맵 사례 2~3개를 제시하라. "
            "A4 2~3페이지 분량. 하위 항목: 3.1 리더십 모델링 사례, 3.2 리더 성장 로드맵 사례. "
            "각 사례에 출처 명기."
        ),
    },
    "학습자료": {
        "title": "리더 대상 학습 자료",
        "instruction": (
            "수집된 논문, HBR 아티클, 온라인 강의를 큐레이션하고 활용 가이드를 제공하라. "
...
def _build_prompt(section_key: str, data: dict) -> str:
    ...
    return f"""당신은 리더십 분야 전문 리서처입니다. 아래 수집된 자료를 바탕으로 보고서 섹션을 한국어로 작성하세요.
...
def run(papers: list, hbr_articles: list, courses: list) -> dict:
    print("[통합 분석 에이전트] Claude API로 섹션 생성 시작...")
    ...
    for key, guide in SECTION_GUIDES.items():
        ...
        prompt = _build_prompt(key, all_data)

        # 시스템 프롬프트에 캐시 적용 (공통 자료 재사용)
        response = client.messages.create(
            ...
            system=[
                {
                    "type": "text",
                    "text": "당신은 한국어로 리더십 분야 연구 보고서를 작성하는 전문가입니다. 수집된 자료를 기반으로 체계적이고 심층적인 내용을 작성합니다.",
...
if __name__ == "__main__":
    ...
    run(_load("papers"), _load("hbr_articles"), _load("courses"))
